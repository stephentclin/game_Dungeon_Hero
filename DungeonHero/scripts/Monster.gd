...
func setup(monster_data, world_position: Vector2, owner: Node) -> void:
	...
	ambush_hidden_time = 1.5 if main != null and main.has_method("has_boon") and main.has_boon("blackout_ambush") else 0.0
	visible = true
	set_physics_process(true)
	set_process(true)
	queue_redraw()

func is_ambush_hidden() -> bool:
	return active and ambush_hidden_time > 0.0
...
func on_wall_contact(inward_direction: Vector2) -> void:
	...
	if data.id == "slime":
		...
		if main != null:
			main.combat_system.spawn_floating_text(global_position + Vector2(-20, -58), _loc("墙弹 +10", "WALL CHARGE +10"), Color("#b6ffcf"), false)
		return
	take_damage(6.0, null)
	knockback_velocity += inward_direction.normalized() * 220.0
	knockback_velocity = knockback_velocity.limit_length(280.0)
	if main != null:
		main.combat_system.spawn_floating_text(global_position + Vector2(-14, -58), _loc("撞墙", "WALL HIT"), Color("#f4ca7c"), false)
...
func die() -> void:
	if not active:
		return
	if main != null and data != null and main.has_method("should_curtain_blast") and main.should_curtain_blast(data.id):
		main.combat_system.spawn_explosion(global_position, 44.0, current_attack() * 0.55, 0.0, {"source": "curtain_blast", "always_hit": true}, true)
	active = false
	visible = false
	velocity = Vector2.ZERO
	if collision_shape != null:
...
func _process(delta: float) -> void:
	...
	if summon_lock_time > 0.0:
		...
		queue_redraw()
	if ambush_hidden_time > 0.0:
		ambush_hidden_time = max(0.0, ambush_hidden_time - delta)
		queue_redraw()
...
func _perform_attack_hit() -> void:
	if data.tags.has("ranged"):
		var critical = false
		if main != null and main.has_method("crit_chance_for"):
			critical = randf() < main.crit_chance_for(data)
		var crit_multiplier = main.crit_damage_multiplier() if main != null and main.has_method("crit_damage_multiplier") else 1.5
		var amount = current_attack() * (crit_multiplier if critical else 1.0)
		...
		if data.id == "archer":
			...
			if archer_shot_count % 3 == 0:
				...
				if main != null:
					main.combat_system.spawn_floating_text(global_position + Vector2(-18, -62), _loc("标记箭", "MARKED ARROW"), Color("#fff2a0"), false)
	...
	var crit_multiplier = main.crit_damage_multiplier() if main != null and main.has_method("crit_damage_multiplier") else 1.5
	var tags = _attack_tags(critical)
	var dealt = main.apply_damage_to_hero(current_attack() * (crit_multiplier if critical else 1.0), self, tags)
...
func _try_devour_low_hp_ally() -> void:
	...
	for ally in main.get_active_monsters():
		...
		var meal = {
			...
			"name": main.monster_display_name(ally.data.id) if main != null and main.has_method("monster_display_name") else str(ally.data.display_name)
		}
		...
		devour_cooldown = 0.8
		if main != null:
			main.combat_system.spawn_floating_text(global_position + Vector2(-24, -78), _loc("吞噬：%s", "DEVOUR: %s") % meal["name"], Color("#ffce77"), true)
		return
...
func _draw() -> void:
	...
	if berserk:
		# Red target ring removed; keep the status text only.
		draw_string(ThemeDB.fallback_font, Vector2(-18, 34), _loc("狂暴", "BERSERK"), HORIZONTAL_ALIGNMENT_LEFT, -1, 12, Color("#ffb058"))
	if pool_empowered:
		draw_arc(Vector2.ZERO, radius + 13.0, 0.0, TAU, 28, Color(0.30, 1.0, 0.74, 0.90), 2.0)
		draw_string(ThemeDB.fallback_font, Vector2(-22, 47), _loc("沼泽强化 +10", "POOL BOOST +10"), HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color("#baffce"))
	# v10.36: Guardian inspire remains mechanical, but the green visual ring/text is hidden.
	if ambush_hidden_time > 0.0:
		draw_arc(Vector2.ZERO, radius + 16.0, 0.0, TAU, 24, Color(0.36, 0.30, 0.62, 0.78), 1.8)
	...
	# Unit name hidden for a cleaner battlefield.
...
func _short_name() -> String:
	match data.id:
		"warrior":
			return _loc("战士", "Warrior")
		"archer":
			return _loc("弓手", "Archer")
		"slime":
			return _loc("史莱姆", "Slime")
		"bomber":
			return _loc("炸弹", "Bomb")
		"shaman":
			return _loc("萨满", "Shaman")
		"ogre":
			return _loc("食人魔", "Ogre")
		_:
			return main.monster_display_name(data.id) if main != null and main.has_method("monster_display_name") else data.display_name
