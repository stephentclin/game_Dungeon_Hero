...
func expire() -> void:
	if not active:
		return
	if main != null:
		main.combat_system.spawn_floating_text(global_position + Vector2(-22, -24), _loc("地形消散", "TERRAIN FADES"), Color(0.72, 0.86, 0.72))
	_remove_from_scene()
...
func ignite(fuse_override := -1.0) -> void:
	...
	if barrel_state == BARREL_STATE_IDLE and main != null:
		main.combat_system.spawn_floating_text(global_position + Vector2(-22, -42), _loc("点燃", "IGNITED"), Color(1.0, 0.42, 0.26), true)
...
func _explode_barrel() -> void:
	...
	if main != null and main.audio_manager != null:
		main.audio_manager.play_sfx("bomb")
	...
	if main != null:
		...
		if hero_multiplier > 1.0:
			main.combat_system.spawn_floating_text(global_position + Vector2(-45, -44), _loc("自爆联动 x%d", "BOMB SYNERGY x%d") % int(hero_multiplier), Color(1.0, 0.72, 0.22), true)
	destroyed.emit(self)
	queue_redraw()
