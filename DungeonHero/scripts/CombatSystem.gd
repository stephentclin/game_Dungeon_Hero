...
func spawn_sprite_sheet_fx(world_position: Vector2, texture: Texture2D, frame_count: int, frame_size: Vector2, draw_size: Vector2, lifetime: float, offset := Vector2.ZERO, modulate := Color.WHITE) -> void:
	if texture == null:
		return
...
func shake(_duration: float, _strength: float) -> void:
	# Disabled: no screen shake effects.
	shake_time = 0.0
	shake_strength = 0.0
...
func _draw() -> void:
	...
	for effect in sprite_fx:
		var ratio: float = clamp(float(effect["life"]) / float(effect["max_life"]), 0.0, 1.0)
		var size: Vector2 = effect["size"]
		var effect_position: Vector2 = effect["position"]
		var effect_offset: Vector2 = effect["offset"]
		var rect: Rect2 = Rect2(effect_position + effect_offset - size * 0.5, size)
		var modulate: Color = effect["modulate"]
		modulate.a *= ratio
		var texture: Texture2D = effect["texture"]
		if effect.has("frame_count"):
			var frame_count: int = max(1, int(effect["frame_count"]))
			var frame_size: Vector2 = effect["frame_size"]
			var progress: float = 1.0 - ratio
			var frame_index: int = clampi(int(floor(progress * float(frame_count))), 0, frame_count - 1)
			var source_rect: Rect2 = Rect2(Vector2(frame_size.x * float(frame_index), 0.0), frame_size)
